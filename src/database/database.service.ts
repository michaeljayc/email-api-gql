...
export class DatabaseService {
  ...
  async insertRecord(database: string, table: string, params: any) {
    try {
      return await this.r
        .db(database)
        .table(table)
        .insert(params, { returnChanges: 'always' });
    } catch (error) {
  ...
  async updateRecord(database: string, table: string, id: string, params: any) {
    return this.r
      ...
      .table(table)
      .get(id)
      .update(params, { returnChanges: 'always' });
  }
          ...
          return item('id')
            ...
            .and(
              item('recipient')('email')
                .eq(query.reference)
                .and(item('recipient')('menu').eq(query.menu))
                .or(
                  item('sender')('email')
                    .eq(query.reference)
                    .and(item('sender')('menu').eq(query.menu)),
                ),
            );
        });
  ...
  async search(
    ...
    if (table === 'messages')
      return this.r
        ...
        .filter(function (item) {
          return item('recipient')('email')
            .eq(reference)
            .or(item('sender')('email').eq(reference))
            .and(
              item('subject').match(keyword).or(item('message').match(keyword)),
            );
        });
    ...
    return this.r
      ...
      .filter(function (item) {
        return item('email')
          .match(keyword)
          .or(item('username').match(keyword))
          .or(
            item('first_name')
              .match(keyword)
              .or(item('last_name').match(keyword)),
          );
      });
  }
}
